<template>
  <div class="nft-cards__media-wrap">
    ...
    <router-link
      ...
      :to="{ name: 'NFTDetails', params: { id: props.metadata.tokenId }}"
    >
...
const loadContent = async () => {
  if (props.metadata) {
    ...
    if (!nftImage.value || props.isBundle) {
      ...
      tokenData = await store.dispatch("setTokenImage", props.metadata);

      // if need extra info for single page, dispatch
      if (props.getInfo) {
        store.commit("setCurrentNFT", {
          ...tokenData,
    ...
    nftData.value = tokenData ? tokenData : null;
  }
};
</script>
